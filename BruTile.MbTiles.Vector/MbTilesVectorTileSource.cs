...
using BruTile.MbTiles.Vector.VectorTileRenderer;
...
public enum VectorStyleKind { Basic, Bright, Dark, Light, Liberty, Toner, Positron, Custom }

/// <summary>
/// An <see cref="ITileSource"/> implementation for MapBox Tiles files
/// </summary>
/// <seealso href="https://www.mapbox.com/developers/mbtiles/"/>
public class MbTilesVectorTileSource : ITileSource
{
    ...
    private readonly VectorStyleKind _style;
    ...
    public MbTilesVectorTileSource(SQLiteConnectionString connectionString, ITileSchema? schema = null, MbTilesType type = MbTilesType.None,
        bool determineZoomLevelsFromTilesTable = false, bool determineTileRangeFromTilesTable = false, VectorStyleKind styleKind = VectorStyleKind.Basic)
    {
    ...
    public async Task<byte[]?> GetTileAsync(TileInfo tileInfo)
    {
        ...
        if (IsTileIndexValid(index))
        {
            ...
            var rawTile = await cn.ExecuteScalarAsync<byte[]>(sql, index.Level, index.Row, index.Col);

            // we now need to decode the PBF
            var pbfTileProvider = new PbfTileSource(rawTile);

            var style = new VectorStyle(_style);
            style.SetSourceProvider("openmaptiles", pbfTileProvider);
