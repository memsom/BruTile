...
public class ClipperBase
{
    ...
    public bool AddPolygon(List<Coordinate> pg, PolyType polyType)
    {
        long num;
        int count = pg.Count;
        if (count < 3)
        ...
        var intPoints = new List<Coordinate>(count)
        {
            new Coordinate {X = pg[0].X, Y = pg[0].Y}
        };
        int num1 = 0;
        for (int i = 1; i < count; i++)
        {
            ...
            if (!ClipperBase.PointsEqual(intPoints[num1], pg[i]))
            {
                ...
                {
                    intPoints.Add(new Coordinate {X = pg[i].X, Y = pg[i].Y});
                }
                else
                {
                    intPoints[num1] = pg[i];
    ...
    public bool AddPolygons(List<List<Coordinate>> ppg, PolyType polyType)
    {
        bool flag = false;
    ...
    private void InitEdge(TEdge e, TEdge eNext, TEdge ePrev, Coordinate pt, PolyType polyType)
    {
        e.next = eNext;
        e.prev = ePrev;
        e.xcurr = pt.X;
    ...
    public static int PointInPolygon(Coordinate pt, OutPt op)
    {
    ...
    internal bool PointIsVertex(Coordinate pt, OutPt pp)
    {
        OutPt outPt = pp;
        do
        {
    ...
    protected static bool PointsEqual(Coordinate pt1, Coordinate pt2)
    {
        if (pt1.X != pt2.X)
        {
            return false;
    ...
    protected bool SlopesEqual(Coordinate pt1, Coordinate pt2, Coordinate pt3, bool UseFullRange)
    {
        if (UseFullRange)
        {
    ...
    protected bool SlopesEqual(Coordinate pt1, Coordinate pt2, Coordinate pt3, Coordinate pt4, bool UseFullRange)
    {
        if (UseFullRange)
        {
