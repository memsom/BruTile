...
internal static class KnownColors
{
    static KnownColors()
    {
        ...
        {
            if (item.GetValue(null) is SKColor colorValue)
            {
                var coloruint = (uint)(colorValue.Alpha << 24 | colorValue.Red << 16 | colorValue.Green << 8 | colorValue.Blue);
                var aRgbString = $"#{coloruint,8:X8}";
                SKnownArgbColors[aRgbString] = colorValue;
            }
        }
    }
    ...
    private static string MatchColor(string colorString, out bool isKnownColor, out bool isNumericColor, out bool isContextColor, out bool isScRgbColor)
    {
        ...
        if (trimmedString.Length is 4 or 5 or 7 or 9 && (trimmedString[0] == '#'))
        {
            isNumericColor = true;
            isScRgbColor = false;
            isKnownColor = false;
    ...
    {
        var trimmedColor = MatchColor(color, out var isPossibleKnowColor, out var isNumericColor, out var isContextColor, out var isScRgbColor);
    ...
    private const int SZeroChar = (int)'0';
    private const int SALower = (int)'a';
    private const int SAUpper = (int)'A';
    ...
    {
        var argbUpper = argbString.Trim().ToUpper(System.Globalization.CultureInfo.InvariantCulture);

        if (SKnownArgbColors.TryGetValue(argbUpper, out var color))
        {
            return color;
        }
    ...
    private static readonly Dictionary<string, SKColor> SKnownArgbColors = new();
}
