...
public class VectorStyle
{
    ...
    public VectorStyle(VectorStyleKind style, double scale = 1)
    {
        ...
        foreach (JProperty jSource in sources)
        {
            ...
            if (sourceDict.TryGetValue("url", out var value))
            {
                source.URL = PlainifyJson(value) as string;
            }

            if (sourceDict.TryGetValue("type", out var value1))
            {
                source.Type = PlainifyJson(value1) as string;
            }

            if (sourceDict.TryGetValue("minzoom", out var value2))
            {
                source.MinZoom = Convert.ToDouble(PlainifyJson(value2));
            }

            if (sourceDict.TryGetValue("maxzoom", out var value3))
            {
                source.MaxZoom = Convert.ToDouble(PlainifyJson(value3));
            }
        ...
        foreach (JObject jLayer in layers)
        {
            ...
            if (layerDict.TryGetValue("minzoom", out var value))
            {
                layer.MinZoom = Convert.ToDouble(PlainifyJson(value));
            }

            if (layerDict.TryGetValue("maxzoom", out var value1))
            {
                layer.MaxZoom = Convert.ToDouble(PlainifyJson(value1));
            }

            if (layerDict.TryGetValue("id", out var value2))
            {
                layer.ID = PlainifyJson(value2) as string;
            }

            if (layerDict.TryGetValue("type", out var value3))
            {
                layer.Type = PlainifyJson(value3) as string;
            }

            if (layerDict.TryGetValue("source", out var value4))
            {
                layer.SourceName = PlainifyJson(value4) as string;
                layer.Source = Sources[layer.SourceName];
            }

            if (layerDict.TryGetValue("source-layer", out var value5))
            {
                layer.SourceLayer = PlainifyJson(value5) as string;
            }

            if (layerDict.TryGetValue("paint", out var value6))
            {
                layer.Paint = PlainifyJson(value6) as Dictionary<string, object>;
            }

            if (layerDict.TryGetValue("layout", out var value7))
            {
                layer.Layout = PlainifyJson(value7) as Dictionary<string, object>;
            }

            if (layerDict.TryGetValue("filter", out var value8))
            {
                var filterArray = value8 as JArray;
    ...
    public void SetSourceProvider(int index, IVectorTileSource provider)
    {
        int i = 0;
    ...
    public void SetSourceProvider(string name, IVectorTileSource provider)
    {
    ...
    public Brush ParseStyle(Layer layer, double scale, Dictionary<string, object> attributes)
    {
        ...
        if (layoutData != null)
        {
            ...
            if (layoutData.ContainsKey("text-offset"))
            {
                ...
                paint.TextOffset = new Coordinate
                    {
                        X = (long)(Convert.ToDouble(value[0]) * scale),
                        Y = (long)(Convert.ToDouble(value[1]) * scale)
                    };
            }
