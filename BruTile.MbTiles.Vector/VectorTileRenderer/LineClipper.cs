...
static class LineClipper
{
    ...
    static OutCode ComputeOutCode(Coordinate p, Rect r) { return ComputeOutCode(p.X, p.Y, r); }

    static Coordinate CalculateIntersection(Rect r, Coordinate p1, Coordinate p2, OutCode clipTo)
    {
        var dx = (p2.X - p1.X);
        var dy = (p2.Y - p1.Y);
        ...
        if (clipTo.HasFlag(OutCode.Bottom))
        {
            return new Coordinate
            {
                X = (long)(p1.X + slopeY * (r.Bottom - p1.Y)),
                Y = (long)r.Bottom
            };
        }
        ...
        if (clipTo.HasFlag(OutCode.Right))
        {
            ...
            {
                X = (long)r.Right,
                Y = (long)(p1.Y + slopeX * (r.Right - p1.X))
            };
        }
        ...
        if (clipTo.HasFlag(OutCode.Left))
        {
            ...
            {
                X = (long)r.Left,
                Y = (long)(p1.Y + slopeX * (r.Left - p1.X))
            };
        }
    ...
    public static Tuple<Coordinate, Coordinate> ClipSegment(Rect r, Coordinate p1, Coordinate p2)
    {
        ...
        if (accept)
        {
            return new Tuple<Coordinate, Coordinate>(p1, p2);
        }
    ...
    static Rect getLineRect(List<Coordinate> polyLine)
    {
    ...
    public static List<Coordinate> ClipPolyline(List<Coordinate> polyLine, Rect bounds)
    {
